# Copied from Dave Cote's (Author's) GitHub repository
# https://github.com/CoteDave/blog/blob/master/Hands-on%20Advanced%20Deep%20Learning%20Time%20Series%20Forecasting%20with%20Tensors/rdr_scorer.py
...
def get_pred_dates(freq, X_pred, date_colname, n_leads):
    ...
    if freq == 12:
        X_pred_date = pd.DataFrame(
            [max(X_pred.index) + relativedelta(months=1) + MonthEnd(0)]
        )
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(months=i) + MonthEnd(0)]
                )
        ...
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(weeks=i)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
        ...
        X_pred_date = pd.DataFrame(
            [max(X_pred.index) + relativedelta(months=3) + MonthEnd(0)]
        )
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(months=i * 3) + MonthEnd(0)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
        ...
        X_pred_date = pd.DataFrame(
            [max(X_pred.index) + relativedelta(months=6) + MonthEnd(0)]
        )
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(years=i * 6) + MonthEnd(0)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
        ...
        X_pred_date = pd.DataFrame(
            [max(X_pred.index) + relativedelta(years=1) + MonthEnd(0)]
        )
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(years=i) + MonthEnd(0)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
        ...
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(days=i)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
        ...
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(minutes=i * 60)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
        ...
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(minutes=i * 30)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
        ...
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(minutes=i * 15)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
        ...
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(minutes=i * 10)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
        ...
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(minutes=i * 5)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
        ...
        if n_leads >= 2:
            for i in range(2, n_leads + 1):
                X_pred_date = X_pred_date.append(
                    [max(X_pred.index) + relativedelta(minutes=i)]
                )
            X_pred_date.columns = [date_colname]
            X_pred_date.index = X_pred_date[date_colname]
        else:
...
class NaiveRandomWalk(BaseEstimator, RegressorMixin):
    ...
    def fit(self, dataset, y_colname, date_colname, n_leads, freq, ci=0.95):
        ...
        start_date = np.min(
            get_pred_dates(freq, dataset[y_colname], date_colname, n_leads)
        )[0]
        end_date = np.max(
            get_pred_dates(freq, dataset[y_colname], date_colname, n_leads)
        )[0]
        ...
        if self.with_drift == False:
            try:
                model = sm.tsa.statespace.SARIMAX(
                    endog=dataset[y_colname],
                    order=(0, 1, 0),
                    trend="n",
                    seasonal_order=(0, 0, 0, 0),
                )
                model = model.fit(method="lbfgs")
                arima_pred = model.get_prediction(
                    start=start_date, end=end_date, dynamic=False
                )
            except:
                arima_pred = model.get_prediction(
                    len(dataset) + 1, len(dataset) + n_leads, dynamic=False
                )
        else:
            try:
                model = sm.tsa.statespace.SARIMAX(
        ...
        arima_pred.index = get_pred_dates(
            freq, dataset[y_colname], date_colname, n_leads
        )[date_colname]
...
class RdR_scorer:
    ...
    def fit(self, ts, y_colname, n_step, y_true, y_pred, model_name="MODEL", freq=12):
        ...
        dtw_distance_rw = slow_dtw_distance(
            y_true_1darray, RandomWalkPred["Y_PRED"].values.ravel()
        )
        ...
        df_scale["RMSE_SCORE"] = (
            1 - rmse_scaler.fit_transform(df_scale[["RMSE"]].values).ravel()
        )
        df_scale["DTW_SCORE"] = (
            1 - dtw_scaler.fit_transform(df_scale[["DTW"]].values).ravel()
        )
        ...
        df_all = pd.concat([df_scale, df], axis=0, ignore_index=True).reset_index(
            drop=True
        )

        rmse_pourc = y_true.copy()
        rmse_pourc["RMSE_POURC"] = df["RMSE"].values / rmse_pourc[y_colname].values
        rmse_pourc["RMSE_ACCURACY"] = 1 - (
            df["RMSE"].values / rmse_pourc[y_colname].values
        )
        ...
        self._def_viz = pd.concat(
            [self._model_df, self._rw_df], axis=0, ignore_index=True
    ...
    def add_rdr(self, rdr_object):
        self._def_viz = pd.concat(
            [self._def_viz, rdr_object._def_viz], axis=0, ignore_index=True
        ).reset_index(drop=True)
    ...
    def plot_rdr_rank(self, models=list([])):
        ...
        models = models.append(
            pd.DataFrame(
                [[0, 0, 1.0, "Perfect Score"]],
                index=[max(models.index) + 1],
        ...
        colors = []
        for value in model_ranking.loc[
            :, "RdR_SCORE"
        ]:  # keys are the names of the boys
            if value < 0:
                colors.append("r")
            elif value <= np.max(
                model_ranking.loc[model_ranking["MODEL_NAME"] == "RandomWalk"][
                    "RdR_SCORE"
                ]
            ):
                colors.append("y")
    ...
    def plot_rdr(
        ...
        ax1.add_patch(
            mpatches.Rectangle(
                (0, 0),
                self._rw_df["DTW"].values[0],
                self._rw_df["RMSE"].values[0],
                alpha=0.1,
                color="green",
                linestyle="--",
                linewidth=1.5,
        ...
        for i in range(0, len(models)):
            ...
            if (
                (model["DTW"].values[0] >= self._rw_df["DTW"].values[0])
                or (model["RMSE"].values[0] >= self._rw_df["RMSE"].values[0])
            ) == True:
                plt.scatter(
                    model["DTW"],
                    model["RMSE"],
