...
class TransformerBoxCox:
    ...
    def fit_transform(self, X, y=None):
        ...
        if min(np.min(X.loc[:, colnames_numerical]).dropna()) <= 0:
            self._translation = (
                abs(min(np.min(X.loc[:, colnames_numerical]).dropna())) * 2
            )
        ...
        for col in range(X.shape[1]):
            ...
            if self.order == 1:
                ...
                if i == 0:
                    ...
                    last_value = np.concatenate(
                        (last_value, np.array(X_diff.dropna().head(1)).reshape(-1, 1)),
                        axis=0,
                    )

                    pred_value = np.array(X[-1, col]).reshape(-1, 1)
                    pred_value = np.concatenate(
                        (pred_value, np.array(X_diff.dropna().tail(1)).reshape(-1, 1)),
                        axis=0,
                    )
                else:
                    diff = np.column_stack((diff, np.array(X_diff2)))
                    last_value2 = np.array(X[0, col]).reshape(-1, 1)
                    last_value2 = np.concatenate(
                        (last_value2, np.array(X_diff.dropna().head(1)).reshape(-1, 1)),
                        axis=0,
                    )
                    last_value = np.column_stack((last_value, last_value2))

                    pred_value2 = np.array(X[-1, col]).reshape(-1, 1)
                    pred_value2 = np.concatenate(
                        (pred_value2, np.array(X_diff.dropna().tail(1)).reshape(-1, 1)),
                        axis=0,
                    )
                ...
                if i == 0:
                    ...
                    last_value = np.concatenate(
                        (last_value, np.array(X_diff2.dropna().head(1)).reshape(-1, 1)),
                        axis=0,
                    )
                    ...
                    pred_value = np.concatenate(
                        (pred_value, np.array(X_diff2.dropna().tail(1)).reshape(-1, 1)),
                        axis=0,
                    )
                else:
                    ...
                    last_value2 = np.concatenate(
                        (
                            last_value2,
                            np.array(X_diff2.dropna().head(1)).reshape(-1, 1),
                        ),
                        axis=0,
                    )
                    last_value = np.column_stack((last_value, last_value2))
                    ...
                    pred_value2 = np.concatenate(
                        (
                            pred_value2,
                            np.array(X_diff2.dropna().tail(1)).reshape(-1, 1),
                        ),
                        axis=0,
                    )
                    pred_value = np.column_stack((pred_value, pred_value2))
            ...
            for col in range(X.shape[1]):
                ...
                if self.order == 1:
                    ...
                    for j in range(0, len(X[:, col])):
                        undiff = np.concatenate(
                            (undiff, np.array(X[j, col] + undiff[j, 0]).reshape(-1, 1)),
                            axis=0,
                        )
                        # print(i,col,j)
                    if i == 0:
                    ...
                    undiff = np.concatenate(
                        (undiff, X_first3 + X_first2 + X_first2 + X_first1), axis=0
                    )
                    for j in range(0, len(X[:, col])):
                        undiff = np.concatenate(
                            (
                ...
                for j in range(0, len(y_pred[:, col])):
                    ...
                    if col == 0:
                        if j == 0:
                            ...
                            y_pred_undiff = np.array(
                                np.sum(self.pred_value_[:, col]) + pred_first
                            ).reshape(-1, 1)
                            # print(y_pred_undiff)
                        else:
                            # print(y_pred_undiff)
                            y_pred_undiff = np.concatenate(
                                (
                                    y_pred_undiff,
                                    np.array(
                                        y_pred_undiff[j - 1, col] + y_pred[j, col]
                                    ).reshape(-1, 1),
                                ),
                                axis=0,
                            )
                        ...
                        if j == 0:
                            ...
                            y_pred_undiff = np.concatenate(
                                (
                                    y_pred_undiff,
                                    np.array(
                                        y_pred_undiff[j - 1, 0] + y_pred[j, col]
                                    ).reshape(-1, 1),
                                ),
                                axis=0,
                            )
                        ...
                        if j == len(y_pred[:, col]) - 1:
                            y_pred_undiff_all = np.concatenate(
                                (y_pred_undiff_all, y_pred_undiff_all2), axis=1
                            )
                        else:
                            pass
